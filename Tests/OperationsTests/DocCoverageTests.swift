...
/// `DocCoverageScannerTests.swift`, against synthetic fixtures; these two
/// tests are the integration check against the real source tree.
@Suite("Public API doc coverage")
struct DocCoverageTests {
    @Test("every public declaration in Sources/Operations has an attached doc comment")
    func operationsSourceIsFullyDocumented() throws {
        let violations = try DocCoverageScanner.scan(directory: "Sources/Operations")
        #expect(violations.isEmpty, Comment(rawValue: "\n" + violations.map(\.description).joined(separator: "\n")))
    }

    @Test("every public declaration in Sources/OperationsCLI has an attached doc comment")
    func operationsCLISourceIsFullyDocumented() throws {
        let violations = try DocCoverageScanner.scan(directory: "Sources/OperationsCLI")
        #expect(violations.isEmpty, Comment(rawValue: "\n" + violations.map(\.description).joined(separator: "\n")))
    }
...
/// Scans a directory of Swift source files for `public` declarations that
/// have no `///` doc comment directly attached to them.
///
/// "Directly attached" means the doc comment's last line is separated from
/// the declaration (or its leading attributes, e.g. `@attached(...)`) by
/// exactly one newline — no blank line in between — matching how this
/// codebase's existing, review-enforced doc comments are written throughout
/// `Sources/Operations` and `Sources/OperationsCLI`.
enum DocCoverageScanner {
    ...
    struct Violation: CustomStringConvertible, Equatable {
        ...
        /// The declaration's name (or, for a `var`/`let` binding a
        /// tuple pattern or an enum case with several elements, its
        /// comma-joined names).
        let name: String
    ...
    /// both `Sources/Operations` and `Sources/OperationsCLI` are flat) for
    ...
    ///   directory cannot be listed, or a `.swift` file within it cannot be
    ///   read as UTF-8.
    static func scan(directory: String) throws -> [Violation] {
        ...
        for file in files {
            ...
            allViolations.append(contentsOf: violations(in: source, filePath: "\(directory)/\(file.lastPathComponent)"))
        }
        return allViolations
    }

    /// Parses `source` and returns every undocumented `public` declaration
    /// found in it, attributing each to `filePath`.
    ...
    /// levels up from `Tests/OperationsTests/DocCoverageTests.swift`.
...
/// Walks a parsed source file, recording every `public` declaration — and
/// every `case` inside a `public enum`, which inherits the enum's access
/// level and carries no modifier of its own — that has no doc comment
...
private final class DocCoverageVisitor: SyntaxVisitor {
    ...
    /// Records a violation if `node` carries an explicit `public` modifier
    /// and has no doc comment directly attached to it.
    ...
    /// Whether `trivia` ends in a doc comment directly attached to the
    /// following declaration: after any trailing indentation, exactly one
    /// newline, then (after any further indentation) a `///` or `/** */`
    /// comment — no blank line in between.
    ...
    /// The comma-joined names of every identifier `node` binds (ordinarily
    /// one, e.g. `public let name: String`).
